  ...
  ({ onSuccess, initialData, isEditMode = false }, ref) => {
    ...
    useEffect(() => {
      if (
        ...
        // Only reset if validation status hasn't been updated
        if (currentValidationStatus === initialData.xvalsta_0?.toString()) {
          // Clean the CIN value before setting it
          const cleanCin = initialData.xcin_0
            ? initialData.xcin_0.trim().toUpperCase()
            : "";
          ...
          reset({
            ...
            xraison_0: initialData.xraison_0 || "",
            xcin_0: cleanCin,
            xvalsta_0: currentValidationStatus,
            montant: initialData.montant || "",
          });
        }
      }
    ...
    const onSubmitForm = async (formData) => {
      ...
      // Prevent modification if the caution is validated (XVALSTA_0 = 2)
      if (initialData?.xvalsta_0 === "2") {
        alert("Modification interdite : cette caution est déjà validée.");
        return;
      }
      ...
      try {
        ...
        if (isEditMode) {
          ...
          submissionData = {
            ...
            xcin_0: (
              ...
              .trim()
              .toUpperCase(),
            xvalsta_0: currentFormValues.xvalsta_0 || formData.xvalsta_0 || "1", // Default to "1" (Non validé) for new cautions
            montant: montantValue,
          };
        ...
        if (initialData && initialData.xnum_0) {
          ...
          await api.put(
            `/xcaution/${encodeURIComponent(initialData.xnum_0)}`,
            submissionData,
            {
              headers: {
          ...
          alert("Caution modifiée avec succès!");
        } else {
        ...
        if (error.response?.data?.errors) {
          ...
          console.log("Submitted CIN value:", submissionData.xcin_0);
          console.log("CIN length:", submissionData.xcin_0?.length);
          console.log(
            "Frontend regex test:",
            /^[A-Za-z]{1,2}\d{4,8}$/.test(submissionData.xcin_0)
          );
    ...
    const handleValidationClick = async (e) => {
      ...
      console.log("Current validation status:", currentValidationStatus); // Debug log
      ...
      try {
        // Get current form values (already got above)
        // const currentValues = getValues();
        ...
        // DO NOT update the form UI - keep it showing "Non" (1)
        // The backend has been updated to "Oui" (2) but form stays unchanged

        console.log("Validation successful - form UI remains unchanged"); // Debug log
    ...
    return (
      <form
        onSubmit={(e) => {
          ...
          // If all validations pass, proceed with normal form submission
        ...
        <div className="sage-form-header">
          <div className="sage-form-header-left">
            ...
            <span className="sage-form-title">
              {isEditMode ? "Modification de Caution" : "Dépôt de Caution"}
            </span>{" "}
            {isEditMode && (
              <span
                style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
            <button
              ...
              disabled={getValues("xvalsta_0") === "2"}
            >
              Validation
            </button>
          ...
          <div className="sage-fields">
            ...
            <div className="sage-row">
              ...
              <input
                ...
                onChange={(e) => {
                  // Transform to uppercase and trim as user types
                  const cleanValue = e.target.value.trim().toUpperCase();
                  e.target.value = cleanValue;
                  setCinHasBeenTouched(true);

                  // Update the form value to ensure validation works with clean value
                  setValue("xcin_0", cleanValue, { shouldValidate: true });

                  // Validate CIN in real-time if not in edit mode
                  if (!isEditMode && !isReadOnly) {
                    if (cleanValue === "") {
                      setIsCinValid(true); // Empty is not invalid, just not filled
                    } else {
            ...
            <div className="sage-row">
              ...
              <select
                ...
                onBlur={() => setFocusField("")}
                value={getValues("xvalsta_0")}
                onChange={(e) =>
                  setValue("xvalsta_0", e.target.value, {
                    shouldValidate: true,
                  })
                }
                disabled={isReadOnly}
